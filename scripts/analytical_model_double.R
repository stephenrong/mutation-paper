...
output_list_v2 <- tibble(
  ...
  "motif_index"=as.numeric(), "motif_ratio"=as.numeric(), 
  "motif_mutrate"=as.numeric(), "motif_ratio_max"=as.numeric())
      ...
      for (i in 1:1) {
        ...
        for (j in 1:length(P_exon)) {
          output_list_v2 <- bind_rows(output_list_v2, c(
            "S"=S, "M_sigma"=M_sigma, "Q_alpha"=Q_alpha, "iter_index"=i, 
            "motif_index"=j, "motif_ratio"=P_exon[j]/P_intron[j],
...
output_list_v2_temp <- filter(output_list_v2, S!=1) %>% 
  group_by(M_sigma, S, Q_alpha) %>% summarise(
    ...
    mutrate_ratio_max = mean(mutrate_ratio_max, na.rm=T)) %>% 
...
output_list_v2_mean <- output_list_v2_temp %>% 
  dplyr::select(M_sigma, S, Q_alpha, motif_ratio_summary, motif_ratio, mutrate_ratio_max)
